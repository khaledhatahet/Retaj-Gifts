...
if(isset($_SESSION["user"])){
  ...
  if(isset( $_GET["indirim"])){
      $indirim = Filtre($_GET["indirim"]);
  
  }else{
      $indirim = "";
  }

  if(isset( $_GET["sontoplam"])){
      $sontoplam = Filtre($_GET["sontoplam"]);
  ...
  if(($toplam != "") && ($kargo != "") && ($indirim != "") && ($sontoplam != "") && ($adet != "") )
  {
    $illeriGetir = mysqli_query($veritabaniBaglantisi,"SELECT * FROM iller ORDER BY sehiradi ASC");
?>
...
<head>
    ...
    <link rel="stylesheet" href="CSS/odeme_sayfasi_style.css">
...
<body>
    ...
    <div class="header" id="head">
        ...
        <div class="UrunlerKismi">
              <h3 style="color: black; font-size:22px; text-align:left; margin-left:50px">Fatura Bilgileri</h3>
              <table style="margin-top: 50px; margin-left:55px" width="700" class="UrunlerTablosu" border="0" cellpadding="0" cellspacing="0">
              <!-- <form action="https://api.whatsapp.com/send/?phone=905385497592&text=Ahmet%3Cbr%3Eaaaaa&app_absent=0"> -->
              <form action="siparisAlindi.php?Toplam=<?php echo $sontoplam ?>&kargo=<?php echo $kargo ?>" method="POST">

                <tr class="satir" height="70">
                  <td width="335"><label>Ad</label><br><input type="text" name="ad" id="ad" class="yazmaAlanlari"></td>
                  <td width="308"><label>Soyad</label><br><input type="text" name="soyad" id="soyad" class="yazmaAlanlari"></td>
                </tr>
                <tr class="satir" height="70">
                  <td width="335"><label>Adres</label><br><input style="height: 36px;" type="text" name="adres" id="adres" class="yazmaAlanlari"></td>
                  <td width="308"><label>İl</label><br>
                  <select name="il" id="il" class="yazmaAlanlari">
                    <option value=""></option>
                  <?php 
                    while($iller = mysqli_fetch_assoc($illeriGetir)){
                      ?>
                      <option value="<?php echo $iller["id"]; ?>"><?php echo $iller["sehiradi"]; ?></option>
                      <?php
                    }
                  ?></select>
                  </td>
                ...
                <tr class="satir" height="70">
                  <td width="335"><label>İlçe</label><br>
                  <select style="height: 36px;" name="ilce" id="ilce" class="yazmaAlanlari">
                  </select></td>
                  <td width="308"><label>Telefon Numarası</label><br><input type="text" name="telefonno" id="telefonno" class="yazmaAlanlari"></td>
                </tr>
                <tr class="satir" height="70">
                  <td colspan="2" width="335"><label>Bize İletmek İstediğiniz Bir Not Varsa Buraya Yazabilirsiniz</label><br><input type="text" name="aciklama" id="aciklama" class="yazmaAlanlari"></td>
                </tr>
                <tr>
                <td colspan="2"  align="left"><input type="submit" class="siparisiGonder" value="Siparişi Gönder"></td>
                </tr>
              </table>
        </div>
    </article>
            ...
            <div class="FiyatKismi">
              <br>
                    ...
                    <table width="280" style="margin-left: 20px;">
                      <tr>
                        <td class="hatciz" width="200">Fiyat(<?php echo $adet ?> Ürün)</td>
                        <td class="hatciz" ><?php echo $toplam ?> TL</td>
                      </tr>
                      <tr>
                        <td class="hatciz" width="60">Kargo ücreti</td>
                        <td class="hatciz"><?php echo $kargo ?> TL</td>
                      </tr>
                      <tr>
                        <td class="hatciz" width="60">İndirim</td>
                        <td class="hatciz"><?php echo $indirim ?> TL</td>
                      </tr>
                      <tr>
                        <td class="hatciz" width="60">Toplam</td>
                        <td class="hatciz"><b><?php echo $sontoplam ?> TL</b></td>
                      </tr>
                    </table>
                    <!-- <button class="odeme2">Siparişi Gönder</button> -->
                    <input type="submit" class="odeme2" value="Siparişi Gönder">
                    </form>
...
<script>
    $(document).ready(function(){
        ...
        $('#il').on('change',function(){
            var query = $(this).val();
            $.ajax({
                url:"ilceleriGetir.php",
                type:"post",
                data:{
                    'il':query
                },
                success:function(data){
                  $('#ilce').html('<option value="" selected></option>' + data);
